...
@Tag(name = "VideoFrameOcr API", description = "VideoFrameOcr相关的增删改查操作")
@RestController
@RequestMapping("/VideoFrameOcr")
@Slf4j
public class VideoFrameOcrController {
    ...
    public ResponseEntity<Boolean> updateByPrimaryKeySelective(@Valid @RequestBody VideoFrameOcr record) {
        ...
        log.info(success ? "Successfully updated VideoFrameOcr" : "Failed to update VideoFrameOcr");
    ...
    public ResponseEntity<Boolean> deleteByPrimaryKey(@PathVariable Long id) {
        ...
        log.info(success ? "Successfully deleted VideoFrameOcr with primary key: id"
                        : "Failed to delete VideoFrameOcr with primary key: id");
        return ResponseEntity.ok(success);
    }
    ...
    public void exportExcel(HttpServletResponse response, @RequestParam(required = false) VideoFrameOcr condition) throws Exception {
        ...
        // 示例：List<VideoFrameOcr> records = condition != null ?
